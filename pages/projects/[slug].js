...
export default function Page({settings, thisPage, navigation, components, image}) {
   return (
      <Layout navigation={navigation} settings={settings} thisPage={thisPage.data}>
         <div className={styles.c_projects}></div>
...
export async function getStaticPaths() {
   const pageResponse = await getAllPages('projects');
...
export async function getStaticProps({params}) { //context?
   ...
   const pageResponse = await getSinglePage(params, 'projects');
   const imageResponse = await getImagePage(pageResponse.data.attributes, 'large', 'projects');
   const componentsResponse = await getComponentsPage(pageResponse.data.attributes, 'projects');

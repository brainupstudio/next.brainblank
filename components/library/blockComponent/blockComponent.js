...
const ComponentBlock = (settings) => {
    ...
    return (
        <div className={'uk-margin-medium-bottom ' + heightClass + backgroundClass + styles.c_block}>
            {settings.data.Title !== null ? <ComponentTitle data={settings.data.Title}/> : false}
            {settings.data.Paragraph !== null ?<ParagraphComponent data={settings.data.Paragraph.Paragraph}/> : false}
        </div>
    )
}
